<?php
    ...
    $sql = "SELECT course_registration.*, courses.name AS course_name, courses.description AS course_description, 
            ...
            WHERE course_registration.user_id = ? AND deadline_materials.to_date >= ? AND deadline_materials.type = 'assignment'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("is", $user_id, $current_date); // 'i' for integer, 's' for string
    $stmt->execute();
    $result = $stmt->get_result();   

    $content = "../Student/show_deadline_materials_assignment_content.php";
